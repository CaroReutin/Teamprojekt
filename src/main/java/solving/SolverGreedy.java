...
/**
 * solver class for greedy.
 */
public class SolverGreedy extends Solver {

  /**
   * solves an instance of the rucksack problem. (solution may not be optimal)
   * Order of the returned list ist the same as the order
   * of the itemList in level
   *
   * @param level level
   * @return returns the list of items that are included in the solution
   */
  @Override
   ...
   * @param amount   Amount of Items available (amount.get(i) must be
   *                 the amount of the item items.get(i))
   * @param startCapacity The capacity of the Rucksack
   * @return Returns an ArrayList of Items that a Greedy Algorithm
   *                 would put in the Rucksack
   */
  public static ArrayList<Item> solveGreedy(final ArrayList<Item> items,
                                            final ArrayList<Integer> amount,
                                            final int startCapacity) {
    int size = items.size();
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (items.get(i).getWeight() > 0 && items.get(i).getValue() > 0) {
        ratio[i] = (double) items.get(i).getValue() / (double)
    ...
    for (int i = 0; i < items.size() - 1; i++) {
      for (int j = i + 1; j < items.size(); j++) {
        if (items.get(indexes[i]).getValue() < items.get(indexes[j])
            .getValue()) {
          int temp = indexes[i];
          indexes[i] = indexes[j];
          indexes[j] = temp;
    ...
    for (int i = 0; i < size - 1; i++) {
      for (int j = i + 1; j < size; j++) {
        if (ratio[i] < ratio[j] || (ratio[j] == -1 && items.get(indexes[j])
            .getWeight() == 0 && items.get(indexes[i]).getValue()
            < items.get(indexes[j]).getValue())) {
          double tmp = ratio[i];
          ratio[i] = ratio[j];
          ratio[j] = tmp;
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < amount.get(indexes[i]); j++) {
        if ((capacity >= items.get(indexes[i]).getWeight() && ratio[i] >= 0)
            || (items.get(indexes[i]).getWeight() == 0 && items.get(indexes[i])
            .getValue() > 0)) {
   ...
   */
  public static ArrayList<Item> sortLikeGreedy(final ArrayList<Item> items) {
    int size = items.size();
    double[] ratio = new double[size];

    for (int i = 0; i < size; i++) {
      if (items.get(i).getWeight() > 0 && items.get(i).getValue() > 0) {
        ratio[i] = (double) items.get(i).getValue()
            / (double) items.get(i).getWeight();
      } else {
        ratio[i] = -1;
      }
    }
    ...
    for (int i = 0; i < size; i++) {
      for (int j = i + 1; j < size; j++) {
        if (ratio[i] < ratio[j] || (ratio[j] == -1 && items.get(j).getWeight()
            == 0 && items.get(i).getValue() < items.get(j).getValue())) {
          Item tmp = items.get(i);
          items.set(i, items.get(j));
          items.set(j, tmp);

...
public class Solver {
   ...
   * @return Returns an ArrayList of items that
   * the greedy algorithm would put in the rucksack
   */
  public static ArrayList<Item> solveGreedy(final Level level) {
   ...
   * @param amount   Amount of Items available
   *                 (amount.get(i) must be the amount of the item items.get(i))
   * @param capacity The capacity of the Rucksack
   * @return Returns an ArrayList of Items that
   * a Greedy Algorithm would put in the Rucksack
   */
  public static ArrayList<Item> solveGreedy(final ArrayList<Item> items,
    ...
    for (int i = 0; i < items.size() - 1; i++) {
      for (int j = i + 1; j < items.size(); j++) {
        if (items.get(indexes[i]).getValue()
    ...
    for (int i = 0; i < size - 1; i++) {
      for (int j = i + 1; j < size; j++) {
        if (ratio[i] < ratio[j] || (ratio[j] == -1
                && items.get(indexes[j]).getWeight() == 0
                && items.get(indexes[i]).getValue()
                    < items.get(indexes[j]).getValue())) {
          double tmp = ratio[i];
          ratio[i] = ratio[j];
          ratio[j] = tmp;
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < amount.get(indexes[i]); j++) {
        if ((capacity >= items.get(indexes[i]).getWeight()
                && ratio[i] >= 0)
                || (items.get(indexes[i]).getWeight() == 0
                && items.get(indexes[i]).getValue() > 0)) {
  ...
  public static ArrayList<Item> sortLikeGreedy(final ArrayList<Item> items) {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = i + 1; j < size; j++) {
        if (ratio[i] < ratio[j] || (ratio[j] == -1
                && items.get(j).getWeight() == 0
                && items.get(i).getValue() < items.get(j).getValue())) {
          Item tmp = items.get(i);
          items.set(i, items.get(j));
          items.set(j, tmp);
